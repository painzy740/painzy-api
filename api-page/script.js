...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const showToast = (message, type = "info", title = "Notifikasi") => {
    ...
    const typeConfig = {
      success: { color: "var(--cyber-success)", icon: "fa-check-circle" },
      error: { color: "var(--cyber-error)", icon: "fa-exclamation-circle" },
      info: { color: "var(--cyber-primary)", icon: "fa-info-circle" },
      notification: { color: "var(--cyber-accent)", icon: "fa-bell" },
    }
  ...
  const populatePageContent = () => {
    ...
    const creator = settings.apiSettings?.creator || "Painzy Labs"

    setPageContent(DOM.pageTitle, settings.name, "Painzy API")
    setPageContent(DOM.wm, `© ${currentYear} ${creator}. Semua hak dilindungi.`)
    setPageContent(DOM.appName, settings.name, "Painzy API")
    setPageContent(DOM.sideNavName, settings.name || "Painzy")
    setPageContent(DOM.versionBadge, settings.version, "v3.0")
    setPageContent(DOM.versionHeaderBadge, settings.header?.status, "⚡ LIVE")
    setPageContent(
      DOM.appDescription,
      settings.description,
      "Next-Generation API Platform - Where Innovation Meets Performance.",
    )
    ...
    if (DOM.apiLinksContainer) {
      ...
      const defaultLinks = [{ url: "https://github.com/painzy-api", name: "⭐ Star Repository", icon: "fab fa-github" }]
    ...
    settings.categories.forEach((category, categoryIndex) => {
      ...
      sortedItems.forEach((item, itemIndex) => {
        ...
        if (item.tags && item.tags.length > 0) {
          ...
          tagsContainer.className = "api-tags"

          item.tags.forEach((tag) => {
            const tagElement = document.createElement("span")
            tagElement.className = "api-tag"
  ...
  const validateModalInputs = () => {
    ...
    // Enable/disable button and add active class
    DOM.modal.submitBtn.disabled = !allFilled
    if (allFilled) {
      DOM.modal.submitBtn.classList.add("btn-active")
    } else {
      DOM.modal.submitBtn.classList.remove("btn-active")
    }
